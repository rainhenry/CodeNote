...
{
	//	为了不改变原始数据，所以复制到新的内存中
	struct tm tmp;
	memcpy(&tmp,pk,sizeof(tm));

	//	格式兼容处理，为了方便计算，把天数改为0基序
	...
	//	以循环形式进行把天到月，月到年的整理
	while(!(
		(tmp.tm_year >= 0) &&																								//	相对年份合法
		((0 <= tmp.tm_mon)&&(tmp.tm_mon <= 11)) &&																			//	月份合法
		((0 <= tmp.tm_mday)&&(tmp.tm_mday <= (GetTotalDayFromMonth(tmp.tm_year + REF_BEGIN_YEAR,tmp.tm_mon + 1) - 1)))		//	天数合法
		))
	{
		//	天向月叠加
		//	如果大于等于0
